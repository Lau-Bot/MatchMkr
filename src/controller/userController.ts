...
class UserController {
    ...
    public async showmatchinfo(req: Request, res: Response) {
        ...
        if (
            await userModel.isjoined(
                Number(req.params.id),
                Number(req.session.userkey)
            )
        ) {
            if (!req.session.auth) {
                req.flash(
            ...
            const matchinfo = await userModel.showmatchinfo(
            ...
            res.render("partials/matchinfojoined", {
                matchinfo: {
                    ...matchinfo,
                    actual_user: req.session.user.usuario,
                },
                volver: true,
            })
        } else {
    ...
    public async joinmatch(req: Request, res: Response) {
        ...
        console.log(req.params.id)
        const result = await userModel.joinmatch(
            Number(req.params.id),
            Number(req.session.userkey)
        )
        console.log(result)
